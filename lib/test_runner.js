...
TestRunner.prototype.run = function () {
    ...
    for (var i = 0; i < abi.length; i++) {
        ...
        if (elem.name === 'TestEvent' && !deepEqual(elem, eventJson)) {
            this.emit('methodsStarted', new Error("TestEvent is not properly formed."));
            return;
        }
    ...
    var prog = this._prog;
    prog.init(this._bin, function(error){
        if(error){
        ...
        async.eachSeries(tests, function (test, callback) {
            ...
            prog.runTx({data: test.sig, to: prog.createdAddress}, function(error, execResult){
                ...
                var excep = execResult.vm.exceptionError;

                if(excep){
                    res.result = false;
                    res.errors.push("Exception: " + excep);
                    that.emit('methodDone', res);
                    return callback();
                }
